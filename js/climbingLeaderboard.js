...
function climbingLeaderboard(scores, alice) {
  const rankedScores = scores.filter((score, i) => score !== scores[i - 1]);
  return alice.map((score) => getScoreIndex(rankedScores, score));
}

/**
 *
 * @param {number[]} rankedScores
 * @param {number} score
 */
const getScoreIndex = (rankedScores, score) => {
  ...
  return binarySearch(rankedScores, 0, rankedScores.length - 1, score) + 1;
};
